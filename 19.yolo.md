...
cnns work using sliding windows...

<img src="img/yolo/cnn_sliding.gif" width="1000"> 
...
<img src="img/yolo/cnn_sliding.gif" width="800">

so what's the **problem** with this?

---

what about objects **much larger** or **much smaller**...
...
**object detection** &rarr; richer task than image classification
...
<img src="img/yolo/yolo_bounding.png" width="1400">
...
each object &rarr; **bounding box** (localization) and a **class label**
...
before YOLO &rarr; dominant approach was **two-stage**,
...
|||
|:-----|:------|
|**Stage 1** | generate region proposals &rarr; candidate bounding boxes |
...
this pipeline produces **accurate** detections but is **slow**
...
none achieve **real-time** speed ($\geq 30$ fps)
...
- map image pixels &rarr; bounding box and class probabilities
...
- original paper &rarr; $S = 7$ \[hence, $7 \times 7 = 49$ cell grid\]
...
<img src="img/yolo/yolo_grids.png" width="1400">

---

<img src="img/yolo/yolo_grids.png" width="1200">


- image divided into grid $S \times S$ (here, $7 \times 7$)
...
- image divided into grid $S \times S$ (here, $7 \times 7$)
- each cell &rarr; responsible for detecting objects 
...
if **centre** of object's bounding box &rarr; falls within cell $(i, j)$
...
then cell $(i, j)$ &rarr; **responsible** for predicting that object
...
- each cell predicts &rarr; $B = 2$ bounding boxes
...
**confidence score** &rarr; how likely is an object in this cell?

--- 

**confidence score** &rarr; how likely is an object in this cell?

during **training** &rarr; compare bounding boxes to **actual ones**
...
<img src="img/yolo/iou_examples.png" width="700"> 
...
each cell also predicts &rarr; $C$ conditional class probabilities
...
complete network output ($S \times S$ grid) &rarr; tensor of shape:

$$S \times S \times (B \times 5 + C)$$

---

for original paper settings:
<br>($S = 7$, $B = 2$, $C = 20$)
...
<img src="img/yolo/yolo_grids.png" width="1000">

each cell produces a **30-dimensional** output vector

---

total raw bounding box predictions per image:
...
<img src="img/yolo/boundbox_0.png" width="1400">

---

<!-- .slide: data-background="white" -->

<img src="img/yolo/boundbox_1.png" width="1400">

- (x,y) is the centre relative to cell
...
multiple cells can predict bounding boxes for the **same object**,

<img src="img/yolo/bounding_boxes_many.png" width="1200">
...
class probability map for "**tanker**",

<img src="img/yolo/probability_tanker.png" width="1200">

---

class probability map for "**tug**",

<img src="img/yolo/probability_tug.png" width="1200">

---

combined **class probability map**,

<img src="img/yolo/probability_map.png" width="1200">
...
at test time &rarr; class-specific confidence score for box $b$, class $c$:

<img src="img/yolo/equations/svgs/equations.001.svg" width="1400">

---

- combining bounding box confidence scores and 
- class probabilities 
...
<img src="img/yolo/combining.png" width="1400">
...
after scoring &rarr; many boxes refer to the **same object**
...
so, we start with multiple bounding boxes...

<img src="img/yolo/car.iou.1.png" width="700">

--- 

<!-- .slide: data-background="white" -->

...and end up with **fewer, relevant** ones

<img src="img/yolo/car.iou.1.png" width="700">
<img src="img/yolo/car.iou.2.png" width="700">

---

<!-- .slide: data-background="white" -->

going back to our ship example...

<img src="img/yolo/combining_NMS.png" width="1400">
...
<img src="img/yolo/yolo_bounding.png" width="1200">
...
- after NMS &rarr; $98$ raw predictions collapse 
- to a **small number** of high-quality detections


---

### Training: The YOLO Loss Function
...
<img src="img/yolo/equations/svgs/equations.002.svg" width="1300">

---


<img src="img/yolo/equations/svgs/equations.003.svg" width="900">
...
- $\lambda_{\text{coord}} = 5$ &rarr; **upweights** localisation loss
...
- $\lambda_{\text{coord}} = 5$ &rarr; **upweights** localisation loss
- $\lambda_{\text{noobj}} = 0.5$ &rarr; **downweights** confidence loss for empty cells
...
- $\lambda_{\text{coord}} = 5$ &rarr; **upweights** localisation loss
- $\lambda_{\text{noobj}} = 0.5$ &rarr; **downweights** confidence loss for empty cells
- **square root** of $w$, $h$ &rarr; penalize errors in small boxes > large
...
- **24 convolutional layers** followed by 2 fully-connected layers
- processes images at $448 \times 448$ resolution
- inspired by GoogLeNet &rarr; $1 \times 1$ bottleneck convolutions
...
- processes images at $448 \times 448$ resolution
- inspired by GoogLeNet &rarr; $1 \times 1$ bottleneck convolutions
- final output &rarr; $7 \times 7 \times 30$ tensor
...
YOLO is more than **6× faster** than Faster R-CNN

---

YOLO is more than **6× faster** than Faster R-CNN

- at **45 fps** &rarr; comfortably exceeds real-time threshold (30 fps)

---

YOLO is more than **6× faster** than Faster R-CNN

- YOLO (**45 fps**) &rarr; exceeds real-time threshold (30 fps)
- Fast YOLO (**155 fps**) &rarr; for high-speed autonomous systems!

---


accuracy gap with two-stage detectors comes from,

---

accuracy gap with two-stage detectors comes from,

- **grid quantisation** &rarr; objects at grid boundaries harder to localise
...
- **grid quantisation** &rarr; objects at grid boundaries harder to localise
- **one class set per cell** &rarr; cannot detect multiple nearby object types
...
- **grid quantisation** &rarr; objects at grid boundaries harder to localise
- **one class set per cell** &rarr; cannot detect multiple nearby object types
- **fixed anchor diversity** &rarr; limited unusual aspect ratios

---

these limitations addressed in later versions,
...
<img src="img/yolo/yolo_architecture.1.jpg" width="1300">
...
<img src="img/yolo/yolo_architecture.1.jpg" width="1300">
<img src="img/yolo/yolo.architecture.2.webp" width="1300">
