...
read the [original paper](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=88137) and [more details](img/object/avoidance/UTK-obstacle_avoidance_slides.pdf)
...
what is "asynchronous" RL?

---

### **asynchonrous** RL

refers to how agent collects data and updates neural network
...
| gets a reward ||
| calculates how to improve  ||
| updates itself before taking the next step ||
...
|:------|:------|
| synchronous | asynchronous|
| one agent takes a step |**multiple** agents in parallel|
| gets a reward | **n-step return** + **advantage** |
| calculates how to improve  ||
| updates itself before taking the next step ||
| **rigid**, **linear** cycle ||

Note:
- workers usually run for a fixed number of steps (e.g., 5 or 20 steps) before reporting back
- total reward ($R$) from a starting state $t$ is calculated by summing the immediate rewards 
- advantage: Was this action better than what I expected to get?
- Each worker calculates its own Advantage based on its own local experience
- the global brain learns a balanced value function that isn't biased by one specific lucky or unlucky ru
...
| one agent takes a step |**multiple** agents in parallel|
| gets a reward | **n-step return** + **advantage** |
| calculates how to improve  | "**Hogwild!**" update |
| updates itself before taking the next step ||
| **rigid**, **linear** cycle ||

Note: 
- When a worker finishes a small batch of experience
- it calculates how the network should change (the gradients) 
- and sends that info to the Global Network.
...
| gets a reward | **n-step return** + **advantage** |
| calculates how to improve  | "**Hogwild!**" update |
| updates itself before taking the next step | global network updates parameters **immediately** |
| **rigid**, **linear** cycle | |

Note:
- doesn't care if other workers are still busy
- workers then "sync" with the new global parameters and go back to work
...
| calculates how to improve  | "**Hogwild!**" update |
| updates itself before taking the next step | global network updates parameters **immediately** |
| **rigid**, **linear** cycle |very **flexible** and **fast** |
||
